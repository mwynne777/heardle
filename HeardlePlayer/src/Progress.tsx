...
export default function Progress({ accessToken, duration, play, uri, seekTo, setDeviceId, setPlay }: ProgressProps) {
    ...
    const updateProgressBar = (increment: number) => {
        timer.current = setInterval(() => {
            setProgress((oldProgress) => {
                ...
                setCurrentTimeStamp(formatTimeStamp(Math.floor((oldProgress + increment)/6.25)))
      ...
      <Fragment>
        <SpotifyPlayer 
            ...
            callback={(data) => {
                ...
                } else if (!data.isPlaying && data.type === 'player_update') {
                    setPlay(false)
                    seekTo(0)
                }
            }}
