...
const game = ({ accessToken, artistId }: GameProps) => {
    ...
    const fetchAutocomplete = async (autocompletePrefix: string) => {
        ...
        if (artistId === null || artistId === undefined) {
            const autocompleteResponse = await fetch(
                `/api/autocomplete?prefix=${autocompletePrefix}`
            )
            newAutoCompleteOptions = (await autocompleteResponse.json())
                .autoCompleteOptions
        } else {
            newAutoCompleteOptions = artistSongs.filter((s) =>
                s.toLowerCase().includes(autocompletePrefix.toLowerCase())
            )
        }
        const previousGuesses = guesses.map((g) => {
            if (g.type !== 'skip') return g.song
        })
        const filteredAutoCompleteOptions = newAutoCompleteOptions.filter(
            (option: string) => !previousGuesses.includes(option)
        )
        dispatch({
            type: 'get-autocomplete-options',
            payload: { options: filteredAutoCompleteOptions },
        })
    }
    ...
    const fetchRandomSong = async () => {
        ...
        const spotifySearchResponse = await spotifyApi.searchTracks(
            randomSongName.song
        )
        dispatch({
            type: 'get-new-song',
            payload: {
                answer: {
                    artist: spotifySearchResponse.tracks.items[0].artists[0]
                        .name,
                    id: randomSongName.song,
                    img: spotifySearchResponse.tracks.items[0].album.images[0]
                        .url,
                    lengthMillis:
                        spotifySearchResponse.tracks.items[0].duration_ms,
    ...
    const getSongsByArtist = async (artistId: string) => {
        ...
        const dedupedAlbums = artistAlbums.items.filter(
            (value, index, self) =>
                index === self.findIndex((t) => t.name === value.name)
        )
        const randomAlbumIndex = Math.floor(
            Math.random() * dedupedAlbums.length
        )
        const promises = dedupedAlbums.map((album) =>
            spotifyApi.getAlbumTracks(album.id)
        )
        const unvalidatedSongs = await Promise.all(promises)
        const songs: string[] = []
        let randomSong: SpotifyApi.TrackObjectSimplified =
            unvalidatedSongs[0].items[0]
        for (
            let albumIndex = 0;
            albumIndex < unvalidatedSongs.length;
            albumIndex++
        ) {
            if (albumIndex === randomAlbumIndex) {
                randomSong =
                    unvalidatedSongs[albumIndex].items[
                        Math.floor(
                            Math.random() *
                                unvalidatedSongs[albumIndex].items.length
                        )
                    ]
            }
            for (
                let songIndex = 0;
                songIndex < unvalidatedSongs[albumIndex].items.length;
                songIndex++
            ) {
                const songDisplayName =
                    unvalidatedSongs[albumIndex].items[songIndex].name
